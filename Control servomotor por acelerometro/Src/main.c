...
double matlab=0; //Recibe dato de matlab
double matlab2=0; //Guarda el dato recibido
...
int compara=0; //Se usa para elegir si se usará matlab o acelerómetro
 char* ang1 = "000"; 
char* ang2 = "000";
char* ang3 = "000";	
char* angs = "000";	
...
void USART2_IRQHandler(void)  //Esta función es la que recibirá los datos obtenidos de la transmisión serial y los guardará en matlab
	{
    ...
    USART2->DR=matlab; // Transmite dato
			matlab2=matlab; //Se guarda en una nueva variable
    SystemCoreClockUpdate(); //Se actualiza cada segundp
...
//		MPU6050_Get_Gyro_RawData(&myGyroRaw); //Angulos / segundo
		...
		x1=((myAccelScaled.x*180)/1000); //Se hace una conversión para que el dato ahora vaya de 0 a 180
		y1=((myAccelScaled.y*180)/1000); //Lo mismo de arriba
		z1=((myAccelScaled.z*180)/1000);
	...
	/////Configuracion para mover el servo////////
	//El sevo está configurado para que cierre en 1 y abra en 180 grados, por lo que es opuesto al movimiento a realizar con el acelerómetro
	 x2=((-0.9964*x1)+179.82); //Se hace una conversión, para que cuando el acelerómetro marque 1, el servo esté en 180, haciendo que se abra
		//Se hace para que, a medida que se vaya aumentando el angulo de inclinación, se vaya cerrando el servo y no al revés
		
		if(x2<=0) { //Define topes para no que no genere problemas con la conversión
			x2=0;
		}
		if(x2>=180) {
			x2=180;
  ...
  tem=((0.005555*x2)+2); //Como el servo se va a mover entre 0 a 1 ms, se usa para que por cada angulo el ancho de pulso aumente 0.005555 ms
	temdelay=20-tem; //Compensa con el tiempo de pulso para que siempre sean 20ms
	GPIOB->ODR=0x3; //Envia un uno durante el tiempo del primer ancho de pulso
	delay(tem);
	GPIOB->ODR=0x0; //Envia un 0 durante la compensación
	delay(temdelay);
  ...
  SSD1306_GotoXY (75, 5); //Define la sección de la OLED
	...
	SSD1306_UpdateScreen(); //Actualiza la pantalla OLED
		...
		else { //Caso cuando se van a recibir datos por transmisión serial
		 sprintf(angs, "%.2f", matlab2); //Convierte el dato obtenido de matlab y lo convierte en una cadena de texto
	angs=dato_chars; //Se guarda lo obtenido en angs
			
		if (matlab2<0){ //Define los topes  del angulo
		matlab2=0;
	}else if(matlab2>180){
		matlab2=180;
	}
	
	tem=((0.005555*matlab2)+1)/3.6694; //En este caso, se divide todo por 3.6694, pues es lo que está tardando en llegar el dato de la transmisión serial
	//Se divide en este valor para que se mantenga la proporción de que cada angulo sean 0.005555 ms
	temdelay=20-tem; //Compensa lo obtenido
	
	GPIOB->ODR=0x3; //Manda un uno durante el ancho de pulso
	delay(tem);
	GPIOB->ODR=0x0; //Manda un cero durante la compensación
	delay(temdelay);
	
	SSD1306_GotoXY (5, 5); //Define la posición a escribir
	SSD1306_Puts ("Grado S1", &Font_7x10, 1); //Escribe lo mostrado, en este caso, indica que 
  SSD1306_GotoXY (5, 25); //Se repite
	SSD1306_Puts ("Grado S2 ", &Font_7x10, 1);
	SSD1306_GotoXY (5, 45);
	SSD1306_Puts ("Profe ponganos 5 :3", &Font_7x10, 1); //Autoexplicativo :)
	SSD1306_UpdateScreen(); //Actualiza la pantalla
	
  SSD1306_GotoXY (75, 5); //Define la posición a escribir
	SSD1306_Puts (angs, &Font_7x10, 1); //Escribe la cadena de texto convertida de lo que se recibe de matlab
	SSD1306_GotoXY (75, 25);
	SSD1306_Puts (angs, &Font_7x10, 1); //Como se está recibiendo un solo dato de matlab, para que el cierre sea gradual y proporcional
	//Se muestra el mismo angulo en los dos servos, pues ambos trabajan con el mismo ancho de pulso

	SSD1306_UpdateScreen(); //Se actualiza la pantalla
		}
